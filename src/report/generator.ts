...
function renderAxeSection(axeFindings: Finding[]): string {
  ...
  lines.push("");
  lines.push(
    "Found by axe-core in every state the personas reached — not just the entry page. These are rule violations, not opinions: each names the rule, the element, and where it appeared.",
  );
  lines.push("");
